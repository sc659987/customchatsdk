...
public class BUser extends BUserEntity {
    ...
    public List<UserThreadLink> getUserThreadLinks(int offset, int limit) {
        UserThreadLinkDao targetDao = daoSession.getUserThreadLinkDao();
        List<UserThreadLink> userThreadLinksNew = targetDao._queryBUser_UserThreadLinksWithLimit(id, offset, limit);
        BThreadDao bThreadDao = daoSession.getBThreadDao();
        QueryBuilder<BThread> queryBuilder = bThreadDao.queryBuilder();
        queryBuilder.join(UserThreadLink.class, UserThreadLinkDao.Properties.BThreadDaoId)
                .where(UserThreadLinkDao.Properties.BUserDaoId.eq(id)).orderDesc(BThreadDao.Properties.LastMessageAdded);
        List<BThread> bThreads = queryBuilder.offset(offset).limit(limit).list();
        Log.i(TAG, String.valueOf(bThreads.size()));
        return userThreadLinksNew;
    }
    ...
    public List<BThread> getPrivateThreadsWithLimit(int offset, int limit) {
        return getPrivateThreads(offset, limit);
    }
